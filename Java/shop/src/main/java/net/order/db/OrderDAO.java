...
public class OrderDAO {
	...
	public List getOrderList(int page,int limit,String id)throws SQLException{
		String sql="select * from (select rownum rnum,ORDER_GOODS_NUM,ORDER_GOODS_NAME,ORDER_GOODS_AMOUNT,ORDER_GOODS_SIZE,ORDER_GOODS_COLOR,ORDER_SUM_MONEY,ORDER_DATE,ORDER_STATUS from (select * from GOODS_ORDER where ORDER_MEMBER_ID=? order by ORDER_DATE desc)) where rnum >= ? and rnum <= ?";
	...
	public int addOrder(OrderBean order, Vector goodsvector) {
		...
		for(int i = 0; i<basketlist.size(); i++) {
			...
			try {
				...
				sql = "insert into goods_order values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,sysdate,?,sysdate,0)";
