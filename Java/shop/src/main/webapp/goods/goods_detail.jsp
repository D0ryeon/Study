...
<table width="960" cellspacing="0" cellpadding="0" border="0" align="center">
	<tr>
		<td colspan="2" align="center"><!-- 상품내용 -->
		<form name="goodsform" aciton="#" method="post">
			<input type="hidden" name="goodsnum" value="${itemArray.getGOODS_NUM}">
			...
			<tr>
				...
				<td width="381" align="center" valign="middle">
					<table width="381" align="center" valign="middle">
						<tr>
					...
					<tr>
						...
						<%
							if(prevpage.getGOODS_NUM() != 0){
								if(price.equals("no")){
						%>
							<a href="Goods_Detail.go?search=prev&gr_goods_num=<%=itemArray.getGOODS_NUM()%>&item=<%=category%>">
						<%
								}else{
						%>
							<a href="Goods_Detail.go?search=prev&gr_goods_num=<%=itemArray.getGOODS_NUM()%>&item=<%=category%>&price=<%=price%>">
						<%
								}
						%>
							[이전상품]
						...
						%>
							...
							<%
								if(price.equals("no")){
							%>
								<a href="Goods_Detail.go?search=next&gr_goods_num=<%=itemArray.getGOODS_NUM()%>&item=<%=category%>">
							<%
								} else {
							%>
								<a href="Goods_Detail.go?search=next&gr_goods_num=<%=itemArray.getGOODS_NUM()%>&item=<%=category%>&price=<%=price%>">
							<%
								}
							%>
								[다음상품]
